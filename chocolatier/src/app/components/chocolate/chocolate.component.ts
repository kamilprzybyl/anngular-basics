...
import { ChocolateData, Data, Prices } from './chocolate-interface';
...
export class ChocolateComponent implements OnInit {
	title: string = 'Markt Pilot Chocolatier';
	subTitle: string = '...your favorite chocolate the cheapest way possible';
	...
	ngOnInit(): void {
		this.chocolateData = {
		  ...
		  console.log(this.chocolateData);
	}
	
	onClick(chocolate: Data) {
		const modalRef = this.modalService.open(ChocolateDetailsComponent)

		modalRef.componentInstance.data = chocolate;
	}
	...
	getLowestPrice(prices: Prices[]): number {
		...
		prices.forEach(item => {
			...
			pricesPer100.push(price / amount * 100);
		});
		let min: number = Math.min.apply(null, pricesPer100);
		return Math.round(min * 100) / 100;
	}

	getAveragePrice(prices: Prices[]): number {
		let sum: number = 0;
		prices.forEach(item => {
			let price: number = item.price;
			let amount: number = item.amount;
	...
	getCheapestShop(prices: Prices[]): String {
		let link: String = "No data";
		const pricesPer100: number[] = [];
		prices.forEach(item => {
			let price: number = item.price;
			let amount: number = item.amount;
			...
			pricesPer100.push(price / amount * 100);
			if (price / amount * 100 === Math.min.apply(null, pricesPer100)) {
				link = item.link;
			}
		});
		return link;
